...
static const char *
_getopt_initialize_a(
    const char *optstring, struct _getopt_data_a *d, int posixly_correct
)
{
            ...
            if (!strncmp(p->name, d->__nextchar, namelen))
            {
                         ...
                         pfound->flag != p->flag || pfound->val != p->val)
                {
                    struct option_list *newp =
                        (struct option_list *)_malloca(sizeof(*newp));
            ...
            strchr(optstring, *d->__nextchar) == NULL)
        {
            if (print_errors)
            {
                if (argv[d->optind][1] == '-')
                    ...
                    fprintf(
                        stderr, "%s: unrecognized option '%c%s'\n", argv[0],
...

    {
        ...
        if (temp[0] == 'W' && temp[1] == ';')
        {
            ...
            if (ambig && !exact)
            {
                ...
                {
                    fprintf(
                        stderr, "%s: option '-W %s' is ambiguous\n", argv[0],
                        d->optarg
                    );
                }
                d->__nextchar += strlen(d->__nextchar);
...
)
{
    ...
    result               = _getopt_internal_r_a(
        argc, argv, optstring, longopts, longind, long_only, &getopt_data_a,
        posixly_correct
    );
...
int
getopt_a(int argc, char *const *argv, const char *optstring) _GETOPT_THROW
{
    return _getopt_internal_a(
        argc, argv, optstring, (const struct option_a *)0, (int *)0, 0, 0
    );
}
int
getopt_long_a(
    int argc, char *const *argv, const char *options,
    const struct option_a *long_options, int *opt_index
) _GETOPT_THROW
{
    return _getopt_internal_a(
        argc, argv, options, long_options, opt_index, 0, 0
    );
}
int
getopt_long_only_a(
...
_getopt_long_r_a(
    int argc, char *const *argv, const char *options,
    const struct option_a *long_options, int *opt_index,
    struct _getopt_data_a *d
)
{
    return _getopt_internal_r_a(
        argc, argv, options, long_options, opt_index, 0, d, 0
    );
}
int
_getopt_long_only_r_a(
    int argc, char *const *argv, const char *options,
    const struct option_a *long_options, int *opt_index,
    struct _getopt_data_a *d
)
{
    return _getopt_internal_r_a(
        argc, argv, options, long_options, opt_index, 1, d, 0
    );
}
...
_getopt_initialize_w(
    const wchar_t *optstring, struct _getopt_data_w *d, int posixly_correct
)
{
    d->__first_nonopt = d->__last_nonopt = d->optind;
    d->__nextchar                        = NULL;
...
int
_getopt_internal_r_w(
    int argc, wchar_t *const *argv, const wchar_t *optstring,
...
)
{
    ...
    if (d->__nextchar == NULL || *d->__nextchar == L'\0')
    {
        ...
        if (d->__ordering == PERMUTE)
        {
            ...
            while (d->optind < argc &&
                   (argv[d->optind][0] != L'-' || argv[d->optind][1] == L'\0'))
                d->optind++;
        ...
        d->__nextchar =
            (argv[d->optind] + 1 +
             (longopts != NULL && argv[d->optind][1] == L'-'));
    }

    if (longopts != NULL &&
        (argv[d->optind][1] == L'-' ||
         (long_only &&
          (argv[d->optind][2] || !wcschr(optstring, argv[d->optind][1])))))
    {
        wchar_t *nameend;
        unsigned int namelen;
        ...
        if (ambig_list != NULL && !exact)
        {
            if (print_errors)
            {
                ...
                ambig_list = &first;
                fwprintf(
                    stderr, L"%s: option '%s' is ambiguous; possibilities:",
        ...
        if (pfound != NULL)
        {
                ...
                else
                {
                    if (print_errors)
                    {
                        ...
                        else
                        {
                            fwprintf(
                                stderr,
                                L"%s: option '%c%s' doesn't allow an "
                                L"argument\n",
                                argv[0], argv[d->optind - 1][0], pfound->name
                            );
                        }
            ...
            else if (pfound->has_arg == 1)
            {
                ...
                else
                {
                    ...
                    {
                        fwprintf(
                            stderr, L"%s: option '--%s' requires an argument\n",
                            argv[0], pfound->name
                        );
                    }
                    d->__nextchar += wcslen(d->__nextchar);
            ...
            wcschr(optstring, *d->__nextchar) == NULL)
        {
            if (print_errors)
            {
                ...
                {
                    fwprintf(
                        stderr, L"%s: unrecognized option '--%s'\n", argv[0],
                        d->__nextchar
                    );
                }
                else
                {
                    fwprintf(
                        stderr, L"%s: unrecognized option '%c%s'\n", argv[0],
                        argv[d->optind][0], d->__nextchar
                    );
                }
            }
    ...
    }
    {
        ...
        if (temp[0] == L'W' && temp[1] == L';')
        {
            ...
            else if (d->optind == argc)
            {
                ...
                {
                    fwprintf(
                        stderr, L"%s: option requires an argument -- '%c'\n",
                        argv[0], c
                    );
                }
                d->optopt = c;
            ...
            if (ambig && !exact)
            {
                ...
                {
                    fwprintf(
                        stderr, L"%s: option '-W %s' is ambiguous\n", argv[0],
                        d->optarg
                    );
                }
                d->__nextchar += wcslen(d->__nextchar);
                ...
                if (*nameend)
                {
                    ...
                    else
                    {
                        if (print_errors)
                        {
                            fwprintf(
                                stderr,
                                L"%s: option '-W %s' doesn't allow an "
                                L"argument\n",
                                argv[0], pfound->name
                            );
                        }
                ...
                else if (pfound->has_arg == 1)
                {
                    ...
                    else
                    {
                        if (print_errors)
                        {
                            fwprintf(
                                stderr,
                                L"%s: option '-W %s' requires an argument\n",
                                argv[0], pfound->name
                            );
                        }
                        d->__nextchar += wcslen(d->__nextchar);
...
)
{
    ...
    result               = _getopt_internal_r_w(
        argc, argv, optstring, longopts, longind, long_only, &getopt_data_w,
        posixly_correct
    );
...
int
getopt_w(int argc, wchar_t *const *argv, const wchar_t *optstring) _GETOPT_THROW
{
    return _getopt_internal_w(
        argc, argv, optstring, (const struct option_w *)0, (int *)0, 0, 0
    );
}
int
getopt_long_w(
    int argc, wchar_t *const *argv, const wchar_t *options,
...
{
    return _getopt_internal_r_w(
        argc, argv, options, long_options, opt_index, 0, d, 0
    );
}
int
_getopt_long_only_r_w(
    int argc, wchar_t *const *argv, const wchar_t *options,
    const struct option_w *long_options, int *opt_index,
    struct _getopt_data_w *d
)
